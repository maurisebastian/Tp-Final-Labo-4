  ...
  savePreferences(): void {
    ...
    forkJoin(requests).subscribe({
      next: (creditsArray: any[]) => {
        ...
        // stats iniciales: cada id empieza en 5
        const genreStats: Record<string, number> = {};
        for (const gid of favoriteGenres) {
          genreStats[String(gid)] = (genreStats[String(gid)] ?? 0) + INITIAL_POINTS;
        }

        const actorStats: Record<string, number> = {};
        for (const aid of favoriteActors) {
          actorStats[String(aid)] = (actorStats[String(aid)] ?? 0) + INITIAL_POINTS;
        }

        this.profileService
          .updateFavoritePreferences(String(active.id), favoriteGenres, favoriteActors, genreStats,actorStats)
          .subscribe({
            next: (ok: boolean) => {
              this.isSaving = false;
